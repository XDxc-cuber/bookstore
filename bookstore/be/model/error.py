...
error_code = {
    ...
    520: "order {} timed out",
    521: "user id {} can't change others' order id {}",
    522: "order id {} status not fit",
    523: "store id {} can't change other store's order id {}",
    524: "",
    525: "",
    526: "",
    527: "",
...
def error_order_timed_out(order_id):
    return 520, error_code[520].format(order_id)

def error_change_others_order(user_id, order_id):
    return 521, error_code[521].format(user_id, order_id)

def error_order_status_not_fit(order_id):
    return 522, error_code[522].format(order_id)

def error_change_other_store_order(store_id, order_id):
    return 523, error_code[523].format(store_id, order_id)
